...
const initializeDatabase = async () => {
  ...
  try {
    ...
    console.log(
      `📁 Creando base de datos '${dbConfig.database}' si no existe...`
    );
    ...
    const configs = [
      ...
      [
        ...
        "90",
        "Días de inactividad antes de revocar dispositivo",
      ],
      [
        "allowed_polygon",
        "[]",
        "Coordenadas del polígono de área permitida (JSON)",
      ],
    ...
    console.log("   Email: jsayasc");
    console.log("   Contraseña: z?0e>FTA");
    console.log("");
...
const checkDatabase = async () => {
  ...
  try {
    ...
    const [devices] = await connection.execute(
      "SELECT COUNT(*) as count FROM authorized_devices WHERE is_active = TRUE"
    );
